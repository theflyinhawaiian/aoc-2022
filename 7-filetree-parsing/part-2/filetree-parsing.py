...
nodes = tree.filter_nodes(lambda x: x.data["type"] == "dir")


size = sum(x.data["size"] for x in tree.children("/"))

availableSpace = 70000000 - size
neededSpace = 30000000 - availableSpace

candidates = filter(lambda x: x.data["type"] == "dir" and neededSpace - x.data["size"] <= 0, nodes)
smallestCandidateDir = reduce(lambda a, b: a if a.data["size"] < b.data["size"] else b, candidates)
print(smallestCandidateDir)